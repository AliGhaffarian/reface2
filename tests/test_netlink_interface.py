...
def test_interface_exists():
    ...
    for name in invalid_interface_names:
        ...
        logger.debug(f"[+] passed on {name=}, expected to be False")
...
def test_change_interface_mac():
    for interface_name in interfaces_to_manipulate:
        ...
        mac_template = mac_templates.templates[random.choice(range(len(mac_templates.templates) - 1))][0]
        mac = str(scapy.all.RandMAC(mac_template))
        ...
        try:
            ...
            logger.warning(f"[!] {e}")
            logger.warning(f"{mac=} {interface_name=}")
        ...
        logger.debug(f"[+] passed {interface_name=} == {mac=}") 
